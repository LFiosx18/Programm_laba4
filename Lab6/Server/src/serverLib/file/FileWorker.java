    ...
    public String fromXmlToObject() {
        try {
            ...
            for (Product val : collectionMng1.getProducts()) {
                ...
                else mes+=("Элемент с именем "+ val.getName() + " не был добавлен в коллекцию!");
            }
            if (mes.equals(""))
                return "Коллекция загружена, сервер готов к работе!";
            else return mes;
        } catch (JAXBException e) {
            return ("Error: " + e.getMessage());
        } catch (NullPointerException e2) {
            return "Error: Не удается найти файл!";
        } catch (FileNotFoundException e3) {
            return ("Error: " + e3.getMessage());
        } catch (Exception e4) {
            return "Error: Ошибка при загрузки коллекции, возможно некоторое атрибуты (ID) указаны некорректно";
        }
    }
